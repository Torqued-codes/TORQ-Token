 ...
 * blockchain.ts
 * ---------------------------------------------------------------------------
 * All ethers.js + TorqToken contract logic + private-key encryption lives
 * here. Nothing in routes/ talks to ethers.js directly.
 */
...
/**
 * Sends a small amount of test ETH from a funded "faucet" account to a
 * newly created custodial wallet, so it can pay gas for mine()/transfer()
 * calls. Required even on a free local testnet - Hardhat still simulates
 * real gas costs, and a wallet with 0 ETH can't submit any transaction.
 */
export async function fundNewWallet(address: string): Promise<void> {
  if (!FUNDER_PRIVATE_KEY) {
    throw new Error("Missing FUNDER_PRIVATE_KEY in server/.env - cannot fund new wallets");
  }
